...
// Provides control ProjectName.control.table.ColumnMenu.
sap.ui.define(['jquery.sap.global', 'sap/ui/core/RenderManager', './library', 'sap/ui/unified/Menu', 'sap/ui/unified/MenuItem', 'sap/ui/Device', './TableUtils'],
	 ...
	 *
	 * <b>Note:</b> Applications must not use or change the default <code>ProjectName.control.table.ColumnMenu</code> of
	 * a column in any way or create own instances of <code>ProjectName.control.table.ColumnMenu</code>.
	 ...
	 * @constructor
	 * @public
	 * @alias ProjectName.control.table.ColumnMenu
	 * @ui5-metamodel This control/element also will be described in the UI5 (legacy) design time metamodel
	 */
	var ColumnMenu = Menu.extend("ProjectName.control.table.ColumnMenu", /** @lends ProjectName.control.table.ColumnMenu.prototype */ { metadata : {
	...
	ColumnMenu.prototype.init = function() {
		...
		this.oResBundle = sap.ui.getCore().getLibraryResourceBundle("ProjectName.control.table");
	...
	ColumnMenu.prototype._updateReferences = function(oParent) {
		...
		if (oParent) {
			jQuery.sap.assert(lazyInstanceof(oParent, "ProjectName/control/table/Column"), "ColumnMenu.setParent: parent must be a subclass of ProjectName.control.table.Column");

			this._oTable = this._oColumn.getParent();
			if (this._oTable) {
				jQuery.sap.assert(lazyInstanceof(this._oTable, "ProjectName/control/table/Table"), "ColumnMenu.setParent: parent of parent must be subclass of ProjectName.control.table.Table");
			}
		}
	};
	...
	ColumnMenu.prototype._addColumnVisibilityMenuItem = function() {
		...
		if (oTable && oTable.getShowColumnVisibilityMenu()) {
			...
			for (var i = 0, l = aColumns.length; i < l; i++) {
				...
				if (bAnalyticalBinding && lazyInstanceof(oColumn, "ProjectName/control/table/AnalyticalColumn")) {
	 ...
	 * @param {ProjectName.control.table.Column} oColumn the associated column to the menu item.
		...
		return new MenuItem(sId, {
			...
			select: jQuery.proxy(function(oEvent) {
				...
				if (bVisible || TableUtils.getVisibleColumnCount(this._oTable) > 1) {
					...
					if (oTable && lazyInstanceof(oTable, "ProjectName/control/table/Table")) {
	 ...
	 * @return {ProjectName.control.table.ColumnMenu} this reference for chaining
	 * @private
	 */
