...
// Provides helper ProjectName.control.table.TableKeyboardDelegate.
sap.ui.define(['jquery.sap.global', 'sap/ui/base/Object', './library', './Row', './TableExtension', './TableUtils', './TableTabbing'],
	function(jQuery, BaseObject, library, Row, TableExtension, TableUtils, TableTabbing) {
		"use strict";
		...
		/**
		 * Delegate for keyboard events of ProjectName.control.table.Table controls.
		 *
		 * @class Delegate for keyboard events of ProjectName.control.table.Table controls.
		 *
		 ...
		 * @constructor
		 * @private
		 * @alias ProjectName.control.table.TableKeyboardDelegate
		 */
		var TableKeyboardDelegate = BaseObject.extend("ProjectName.control.table.TableKeyboardDelegate", /* @lends ProjectName.control.table.TableKeyboardDelegate */ {
			...
			constructor: function(sType) {
				...
				if (sType === TableExtension.TABLETYPES.ANALYTICAL) {
					...
					this.onkeydown = function(oEvent) {
						...
						if (oEvent.keyCode == jQuery.sap.KeyCodes.TAB && this._getKeyboardExtension().isInActionMode() && $TargetTD.find(
								'.sapUiTableTreeIcon').length > 0) {
		...
		TableKeyboardDelegate._restoreFocusOnLastFocusedDataCell = function(oTable, oEvent) {
			//
			var lastInput = $(oEvent.currentTarget).find("input").last();
			oTable._getKeyboardExtension()._setSilentFocus($(lastInput));
		...
		TableKeyboardDelegate.prototype.onmouseup = function(oEvent) {
			...
			this._getKeyboardExtension().setActionMode($Dom.length > 0, {
				$Dom: $Dom,
				event: oEvent
			});
		};
		...
		TableKeyboardDelegate.prototype.onfocusin = function(oEvent) {
			...
			if ($target.hasClass("sapUiTableOuterBefore") || $target.hasClass("sapUiTableOuterAfter") || (oEvent.target != this.getDomRef(
					"overlay") && this.getShowOverlay())) {
			...
			if (!bActionMode &&
				...
				if ($this.find(".sapUiTableCtrl td:focus").length > 0) {
					this._getKeyboardExtension().setActionMode(true, {
						$Dom: $this.find(".sapUiTableCtrl td:focus").find(":sapFocusable")
					});
					oEvent.preventDefault();
		...
		TableKeyboardDelegate.prototype.onsaptabprevious = function(oEvent) {
			...
			if (this._getKeyboardExtension().isInActionMode()) {
				...
				TableTabbing.onTabPrevious(oEvent, this);
		...
		TableKeyboardDelegate.prototype.onsaptabnext = function(oEvent) {
			...
			if (this._getKeyboardExtension().isInActionMode()) {
				...
				if (!bTargetIsTreeIcon || !bCellHasElements) {
					...
					// var inputs = $(oEvent.target).closest("tr").find("input");
					// var bFlag = false;
					// for (var i = 0; i < inputs.length - 1; i++){
					// 	if (inputs[i] === oEvent.target){
					// 		this._getKeyboardExtension()._setSilentFocus($(inputs[i + 1]));
					// 		bFlag = true;
					// 	}
					// }
					// if (!bFlag){
					// 	var nextRow = $(oEvent.target).closest("tr").next();
					// 	var firstInput = nextRow.find("input").first();
					// 	this._getKeyboardExtension()._setSilentFocus($(firstInput));
					// }
					oEvent.preventDefault();
				}
			} else {
