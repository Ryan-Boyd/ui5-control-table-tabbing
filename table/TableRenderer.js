...
//Provides default renderer for control ProjectName.control.table.Table
sap.ui.define(['jquery.sap.global', 'sap/ui/core/Control', 'sap/ui/core/library', 'sap/ui/core/theming/Parameters', 'sap/ui/Device',
		'./library', './TableUtils', 'sap/ui/core/Renderer', 'sap/ui/core/IconPool'
	],
	function(jQuery, Control, coreLibrary, Parameters, Device, library, TableUtils, Renderer, IconPool) {
		...
		TableRenderer.renderColRowHdr = function(rm, oTable) {
			...
			oTable._getAccRenderExtension().writeAriaAttributesFor(rm, oTable, "COLUMNROWHEADER", {
				enabled: bEnabled
			});
			...
			if (iFixedBottomRowCount > 0) {
				...
				if (oBinding) {
					if (oTable._iBindingLength >= iVisibleRowCount) {
						...
						bIsPreBottomRow = (iFirstVisibleRow + iIndex) == (oTable._iBindingLength - iFixedBottomRowCount - 1) && (oTable.getFirstVisibleRow() +
							iIndex) < oTable._iBindingLength;
					}
				}
		...
		TableRenderer.renderTableCtrl = function(rm, oTable) {
			...
			if (oTable._iTableRowContentHeight && (sVisibleRowCountMode == VisibleRowCountMode.Fixed || sVisibleRowCountMode ==
		...
		TableRenderer.renderTableControl = function(rm, oTable, bFixedTable) {
			...
			this.renderTableControlCnt(rm, oTable, bFixedTable, iStartColumn, iEndColumn, false, false, iFixedRows, aRows.length -
				iFixedBottomRows);
		...
		TableRenderer.renderTableControlCnt = function(rm, oTable, bFixedTable, iStartColumn, iEndColumn, bFixedRow, bFixedBottomRow, iStartRow,
			iEndRow) {
			rm.write("<table");
			...
			for (var row = iStartRow, count = iEndRow; row < count; row++) {
				this.renderTableRow(rm, oTable, aRows[row], row, bFixedTable, iStartColumn, iEndColumn, false, aVisibleColumns, bHasOnlyFixedColumns,
					mTooltipTexts, bSelectOnCellsAllowed);
			}
		...
		TableRenderer.renderTableRow = function(rm, oTable, oRow, iRowIndex, bFixedTable, iStartColumn, iEndColumn, bFixedRow, aVisibleColumns,
			bHasOnlyFixedColumns, mTooltipTexts, bSelectOnCellsAllowed) {
			rm.write("<tr");
			if (oRow._bDummyRow) {
		...
		TableRenderer.renderTableCellControl = function(rm, oTable, oCell, bIsFirstColumn) {
			if (TableUtils.Grouping.isTreeMode(oTable) && bIsFirstColumn) {
				...
				oTable._getAccRenderExtension().writeAriaAttributesFor(rm, oTable, "TREEICON", {
					row: oRow
				});
				rm.write(">&nbsp;</span>");
			}
